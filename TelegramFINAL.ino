...
//--------------------------------------------------------------------------
const char* ssid = "Tenda_EA1200";
const char* password = "mecatronica2022";
String idChat = "561549552";//ID_Chat se obtiene de telegram
//Token de Telegram BOT se obtenienen desde Botfather en telegram
String token = "5129316272:AAE-s8LA113-afWVV4LKtRZ40ifhCO9uQBA";
//--------------------------------------------------------------------------
...
void manejarMensajes(int nuevoMensajes){
  ...
  if (text == "/flash") 
  {
    ...
    flashEstado = !flashEstado;
    digitalWrite(flashLed, flashEstado);
    if(flashEstado==0){bot.sendMessage(idChat, "Flash apagado", "");}
    else{bot.sendMessage(idChat, "Flash encendido", "");}
  }
  if (text == "/fotofh") 
  {
    //Toma una foto con flash
    envioFoto = true;
  ...
  if(text == "/powerPC")
  {
    //verifica el estado
    estadoPC = !estadoPC;
    digitalWrite(relay1,!estadoPC);
    if(estadoPC==0){bot.sendMessage(idChat, "PC apagada", "");}
    else{bot.sendMessage(idChat, "PC encendida", "");}
   }
  //  if(text == "/powerLUZ")
  //  {
  //    //verifica el estado
  //    estadoLUZ = !estadoLUZ;
  //    digitalWrite(relay2,estadoLUZ);
  //    if(estadoLUZ==0){bot.sendMessage(idChat, "LUZ apagada", "");}
  //    else{bot.sendMessage(idChat, "LUZ encendida", "");}
  //  }
  if(text == "/giroXAnti")
  {
    //verifica maximo
    if(servo1Pos <= 165)
    {
      servo1Pos += 15;
      servo1.write(servo1Pos);
      bot.sendMessage(idChat,"Girando 15° antihorario", "");
    }
    else{bot.sendMessage(idChat,"Posición máxima alcanzada", "");}
  }
  if(text == "/giroXHora")
  {
    //verifica minimo
    if(servo1Pos >= 15)
    {
      servo1Pos -= 15;
      servo1.write(servo1Pos);
      bot.sendMessage(idChat,"Girando 15° horario", "");
    }
    else{bot.sendMessage(idChat,"Posición mínima alcanzada", "");}
  }
  if (text== "/estado")
  {
    String estados = "Los estados del sistema son" ": \n";
    if(estadoPC==0){estados += "PC: Apagada \n";}
    else{estados += "PC: Encendida \n";}
    //    if(estadoLUZ==0){estados += "LUZ: Apagada \n";}
    //    else{estados += "LUZ: Encendida \n";}
    if(flashEstado==0){estados += "Flash: apagado \n";}
    else{estados += "Flash: Encendido \n";}
    bot.sendMessage(idChat, estados, "");
  }
    ...
    ayuda += "/powerPC : Enciende la PC del laboratorio \n";
    //    ayuda += "/powerLUZ : Enciende la luz del laboratorio \n";
    ayuda += "/giroXHora : giro horario 30° \n";
    ayuda += "/giroXAnti : giro antihorario 30° \n";
    ayuda += "/estado : Muestra el estado de todas las salidas \n";
